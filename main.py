...
from locale_menu import do_locale
...
def do_install(s, selected, users, tz, locale):
    if not selected:
        notice(s, ["No packages selected.", "Go back and pick something.", "", "press any key..."])
        return
    ...
    while True:
        ...
        if choice == len(pkgs) + 1:
            run_install(s, pkgs, users, tz, locale)
            return


def run_install(s, pkgs, users, tz, locale):
    write_config(pkgs, users, tz, locale)
    h, w = s.getmaxyx()
    s.erase()
    header(s)
...
def main(s):
    ...
    while True:
        descs = [
            ...
            locale[0] if locale else "not set",
        ...
        if action == "packages":
            ...
            do_install(s, selected, users, tz, locale)
        elif action == "quit":
...
try:
    ...
    pass
print("\nStainless-Installer exited. You can always return to installation by running: stainless-install\n") 